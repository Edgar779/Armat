...
import {Icon, CreateCancel, ErrMessage } from 'components';
...
export const NoteModal = ({ handleOpenClose, info, addNote, orgId, orgInfo, modalNote, eventInfo, eventSponsor, editOr }) => {
    ...
    const handleChange = (ev) => {
        setDesc(ev.target.value), setErr('');
    };
            ...
            {modalNote && (
                <div className={classes.editButtonWrapper}>
                    <button onClick={handleView}>
                        <Icon name={SVGNames.EditIcon} width={'24px'} height={'24px'} style={{marginRight:'4px'}} />
                        Edit
                    </button>
                </div>
            )}
