...
export const ProfileSettings = ({ info }) => {
    ...
    const handleChangeButton = () => {
        ...
        if (edit === 'Save') {
            if (!validEmail && inputs.fullName && inputs.phoneNumber && inputs.email) {
                const data = {
    ...
    const editSuccess = FindSuccess('EDIT_MY_PROFILE');
    const loader = FindLoad('EDIT_MY_PROFILE');
    const backErr = FindError('EDIT_MY_PROFILE');
    const err = backErr.length && backErr[0].type;
    const errMessageText = validEmail ? validEmail : backErr.length && backErr[0].type === 'EDIT_MY_PROFILE' ? 'Email was exist' : error;
    ...
    const handleChange = (e) => {
        ...
        err && err.length && dispatch(httpRequestsOnErrorsActions.removeError('EDIT_MY_PROFILE'));
        ...
        <div className={classes.profileSettings}>
            ...
            <div style={{ marginTop: '30px' }} className={classes.profileSettingsBodyWrapper}>
                <ProfileBody
                    ...
                    IconType={info && info.auth.role === 'Organizer' ? 'Organizer' : 'User'}
                ...
                <ProfileBody
                    ...
                    errorText={
                        error === 'email'
                            ...
                            ? 'Email is not valid'
                            : backErr.length && backErr[0].type === 'EDIT_MY_PROFILE'
                            ? 'Email was exist'
                            : false
                    }
                    error={
